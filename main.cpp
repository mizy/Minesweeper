...
public:
    void Reset();
    Grid(){NeighborBombNums=0;hasBomb=false;GridIcon=' ';CurrentGridStat=CLOSED;}              //构造函数初始化
...
{
    if (CurrentGridStat==CLOSED){return '-';}                                                  //根据状态显示是否为真实Icon
    if (CurrentGridStat==OPENED){return GridIcon;}
    if (CurrentGridStat==SIGNED){return '!';}
    return 0;
};
 ...
 {                                                                                                     //定义棋盘类
    ...
    public:
        void Reset();
        Panel(){iBombNums=0;iSignedNums=0;iOpenedNums=0;}
        void arrangeBombs();                                                   
 ...
 { 
	 int x,y;
	 iBombNums=0;iSignedNums=0;iOpenedNums=0;
	 for(x=1;x<=SIZE;x++)
		 for(y=1;y<=SIZE;y++)
		 { GridSet[x][y].Reset();};
 }
...
int Panel::judgeGameStat ()                                                                              //判断状态是否succ or fail
 {
	...
	for(x=1;x<=SIZE;x++)
	{
		for(y=1;y<=SIZE;y++)
		{
			if (GridSet[x][y].ishasBomb()) 
			{
				if (GridSet[x][y].getCurrentGridStat()==SIGNED){iSignedNums=iSignedNums+1;};                 //标识数加一
				if(GridSet[x][y].getCurrentGridStat()==OPENED) {return FAIL;};                               //返回失败
				
			}
			   else
			   {
        
				if (GridSet[x][y].getCurrentGridStat()==OPENED){iOpenedNums=iOpenedNums+1;};                 //正确数加一

			   };
		};
	};
	if ((iSignedNums==iBombNums)||(iOpenedNums==(SIZE*SIZE-iBombNums))){ return SUCC;}              // 剩余棋子数等于雷数返回成功
	else {return ONGO;}                                                                             //  返回继续
		...
		while (n<iBombNums)                                                   
		  {  
			...
			{  
				...
				if(x<=SIZE)        {GridSet[x+1][y].setNeighborBombNums();};                       //周围雷数加一
				if(x<=SIZE&&y<SIZE){GridSet[x+1][y+1].setNeighborBombNums();};
				if(x<=SIZE&&y>1)   {GridSet[x+1][y-1].setNeighborBombNums();};
				if(y>1)           {GridSet[x][y-1].setNeighborBombNums();};
				if(y<=SIZE)        {GridSet[x][y+1].setNeighborBombNums();};
				if(x>1)           {GridSet[x-1][y].setNeighborBombNums();}
				if(x>1&&y<=SIZE)   {GridSet[x-1][y+1].setNeighborBombNums();};
				if(x>1&&y>1)      {GridSet[x-1][y-1].setNeighborBombNums();};
                n=n+1;
			};
		};
 ...
 void Panel::displayPanel()                                                                        //显示棋盘函数
 {
	    int x,y;
		cout<<"   y";
	    for(x=1;x<=SIZE;x++){cout<<" "<<x;};                                                        //打印边框
		cout<<endl;
		for(x=1;x<=SIZE;x++){cout<<"____";};
		cout<<endl;
		...
		while(i<=SIZE*SIZE)
		{
        	 ...
        	 switch(menu)                                                                   //选择menu的选项                                     
			 {
			     ...
			     case 2:
					   ...
					   break;
				 case 3:
					 cout<<endl<<"还剩下“"<<aPanel.getRestNums ()<<"”个雷没有被标记。"<<endl;
					 break;
			     case 4:
					   start();
					   break;
